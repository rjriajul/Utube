...
def map(pos):
    if pos == 1:
        button = [[InlineKeyboardButton(text="-->", callback_data="update+2")]]
...
@UtubeBot.on_message(
    filters.private & filters.incoming & filters.command(["update", "up"]), group=2
)
...
update_callback_filter = filters.create(
    lambda _, __, query: query.data.startswith("update+")
)
